...
ALTER TABLE `videos_web` ADD UNIQUE (`video_id`);
ALTER TABLE `videos_web` ADD COLUMN `is_expired` INT(11) DEFAULT '0';
ALTER TABLE `videos_web` ADD COLUMN `check_expire_time` BIGINT(20) DEFAULT '0';
ALTER TABLE `videos_web` ADD COLUMN `vurl` VARCHAR(256) DEFAULT '';
...
CREATE TABLE IF NOT EXISTS `comments_v3` (
    `id` INT(11) NOT NULL AUTO_INCREMENT,
    `user_id` BIGINT(20) NOT NULL COMMENT '发布者ID',
    `user_name` VARCHAR(128) NOT NULL COMMENT '发布者名称',
    `user_avatar` VARCHAR(258) NOT NULL COMMENT '发布者头像',
